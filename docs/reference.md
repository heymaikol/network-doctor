...
The 24 KiB payload is self-labelling filler, not guaranteed harmless to an application; TLS targets get a record header in front so the TLS server reads it instead of resetting on the first byte. Unknown-protocol targets omit this probe by default; `--check path_mtu` explicitly permits the payload and narrows the run to PMTU and its prerequisites, following the usual `--check` semantics. Known-protocol defaults retain it. A profile's inherited check list is not an explicit PMTU request. This is the only probe that sends bulk data: when selected, under `--watch` that is 24 KiB once every 5 seconds.
...
The bottom bar is not a short copy of that table. It carries only what the region on screen needs: how to move in it, what the primary key does to the selected row, and how to leave it, followed by a fixed tail of `space` (Actions), `?` (help) and `q` (quit). Every other binding above stays live and stays unadvertised, because the Actions menu lists what applies right now and `?` lists everything. On a terminal too narrow for one row the bar wraps rather than truncating, so no chip is ever silently dropped, and under height pressure it is replaced by a shorter bar that keeps only the current region's navigation rather than being cut.

The `?` cheatsheet is sectioned by the same groups the Actions menu sorts into (`Move`, `This run`, `This network`, `Drill down`, `Report`, `Session`), then the output viewer's own keys. The drill-down tools are listed under `Drill down` rather than mixed into the built-in actions, so the installed binaries are visibly a separate group from the keys the TUI itself owns.
...
The rows are grouped by what the action acts on, under headings in a fixed order: `This run` (the diagnosis and its evidence: full output, switch/cancel job, expand checks, explain why, incidents, retest), `This network` (the network map, open device, diagnose service, rescan network, SSH login), `Drill down` (the installed tools), `Report` (copy, save) and `Session` (restart, theme, help, quit). The order is the distance from the question the menu answers, "what do I do about this run?", so the next troubleshooting step is always above the housekeeping. Within a group the rows keep the cheatsheet's order, so state can move a row between groups but never inside one. The headings are drawn from the rows, so an empty group prints nothing, they are never selectable, and `enter` only ever runs an action. A group whose rows are scrolled out of view still labels the window it opens in; on a terminal too short for the labels to explain more rows than they cost, the menu drops them and spends every line on actions. It is discovery only: no probe, diagnosis, report, or exit code changes with it.
...
`v` answers "I cannot reach the printer" in two steps, so neither one has to be known in advance. With no earlier LAN scan it confirms and runs unprivileged `nmap -sn` across the source address's `/24`; afterward it shows or hides the newest cached map without probing again. A cached map is a snapshot, not live network state, and the line under the panel title says which it is: `Live scan` while a sweep runs, `Cached snapshot` once one has finished, followed by how long ago it was captured, the fixed capture instant, and the outcome when the scan did not complete. While a sweep runs, and after one fails, is canceled, or times out, the panel also names the earlier successful scan still in the job ring. The map's help bar routes to a fresh one: `space actions: rescan`. Use Actions menu **Rescan network** for a fresh measurement. Use Retest after changing networks so the diagnostic source and LAN context are rebuilt together.

Discovery finds a device only if it accepts or refuses a TCP connect on port 80 or 443. This machine is listed separately rather than as a device to diagnose, and an address gains a name when mDNS, reverse DNS, or an `ssh_config` alias supplies one. Failed, canceled, and timed-out snapshots keep any observed hosts visible and label them as partial results; the devices on screen are always the displayed scan's own, never an earlier scan's carried forward. Earlier scan output remains in the normal bounded job ring until ordinary eviction.
...
`status` is one of `PASS`, `WARN`, `FAIL`, `SKIP`, `N/A`, or `INCOMPLETE`. `target` is `null` in generic (no-target) mode. `ms` is the check's wall time truncated to milliseconds but floored at `1`, so `0` means the check never ran. Optional per-check fields (`cause`, `address_families`, `fix`, `addrs`, `resolver_targets`, `selected_ip`, `source`, `iface`, `network`, `portal`, `attempts`, `routes`, `connect_cleartext`) are omitted when empty. `resolver_targets` lists the DNS service addresses recorded from Go's resolver `Dial` hook as they were dialed; a single lookup's addresses are sorted and deduplicated, but `dns_public`'s failover across resolvers appends each attempt's list in dial order, so the field is not guaranteed globally sorted or unique. It proves that those servers were dialed, not which one returned a response or supplied any particular address: Go resolves A and AAAA as independent queries that fail over independently, so one hostname lookup can be answered by more than one server. It is absent when a lookup was satisfied without dialing DNS, including a hosts-file result reached before any query. The `detail` text follows the same rule and credits the answer to no server, however few were dialed: one reads `resolver tried: …` and two or more read `resolvers tried: …`. A single dialed server is not provenance either, because a host configured `hosts: dns files` reads its hosts file after a query that came back with nothing, so a lookup there can dial one server, be answered by none, and still return addresses. `internet_tcp.address_families` records the independently tested IPv4 and IPv6 state as `reachable` or `unreachable`. `target_tcp.address_families` records the same states for a dual-stack target after comparison with the host's independent family paths. Under `--iface` or an explicit source address, a family the selected source has no address for is never dialed and its key is omitted, meaning untested rather than unreachable. A target family is also omitted when the target publishes it but the host did not prove a usable local path for it. A selection leaving no usable family at all reports the whole egress row as `N/A`, as the QUIC and encrypted-DNS rows already do. A configured egress family whose path fails while the other succeeds warns with `ipv4_unreachable` or `ipv6_unreachable`. Failed QUIC, encrypted-DNS, proxy, and TLS checks populate `cause` so automation can distinguish failure stages without parsing `detail`; QUIC uses `timeout` or `quic_handshake_failure`, encrypted DNS uses `timeout` or `encrypted_dns_unavailable`, while TLS values include `certificate_expired`, `certificate_not_yet_valid`, `hostname_mismatch`, `untrusted_issuer`, `tls_handshake_failure`, `tcp_unreachable`, `timeout`, and `connection_closed`. Failed direct egress may use `no_default_route`, `gateway_unreachable`, `selected_path_failed`, or `preferred_route_failed`, read from the local routing and neighbor tables on Linux, macOS, and Windows. When both families fail, the cause comes from a family that has default routes to describe: a host with no IPv4 default route at all is not reported as routeless when the IPv6 routing it actually uses is what broke. macOS route entries carry no preference metric, so `preferred_route_failed` comes only from Linux and Windows, and `gateway_unreachable` needs a neighbor cache entry that exists and shows the next hop unresolved, so an unseen next hop leaves selection-only metadata rather than a gateway failure. The legacy IDs `selected_path_failed` and `preferred_route_failed` mean a default route or a metric-preferred default was present after reference connectivity failed. Neither establishes a failed route, a healthy gateway, a working alternate, or the location of the outage; they do not independently justify `local_egress_failure`. Within one family, observed gateway failure takes precedence over metric preference. Across failed families, a local defect in one does not explain an unknown failure in another: selection-only or unavailable classification keeps the overall result unlocalized. Both families having localizing evidence, or a sole attempted family having it, preserves localization. Unlocalized findings retain observed unreachable and tunneled routes as context without raising causal confidence. The `portal` object marks HTTP interception corroborated by both fixed connectivity endpoints, and includes `redirect_url` only when one of those responses supplied a valid HTTP(S) sign-in URL; where both did, the first endpoint in netdoc's fixed order wins, so the value never depends on which request finished first. The app displays that URL but never opens it. A single endpoint answering unexpectedly leaves the object absent and warns instead; that row's `cause` is `connectivity_check_unexpected_response` when the discrepancy is the whole warning, and its `detail` says which part of the answer differed, an unexpected status or an unexpected response body under the documented status. A row that is also slow, losing addresses, or missing an address family is describing the path itself and records the path's cause or none at all. `connect_cleartext` is `true` on a working `proxy_connect` row whose environment proxy used a plaintext HTTP CONNECT, meaning the destination hostname reached the proxy without TLS on the client-to-proxy hop; it describes the proxy transport the configuration selected and makes no claim that anything on the path read the name. It is recorded only for a tunnel that succeeded. It is omitted when the observation was not recorded, which covers an `https://` proxy that wrapped the CONNECT in TLS, a SOCKS5 proxy, a non-proxy row, a proxy the run never reached, and a CONNECT the proxy refused after the destination hostname was already on the wire. An absent field never means a TLS hop was confirmed, and never means that no cleartext hostname was sent. The observation never changes the row's status: a successful row keeps the status it earned, which is `PASS` unless an existing dial warning such as high latency or an ambiguous source interface already made it `WARN`. The fact is carried as this additive observation and, for a person, as a hedged `fix` line that names the exposed hop without asserting an `https://` endpoint the probe did not test; that line is shown on the successful row even though ordinary passing rows print no `fix`. Field names and the status vocabulary are stable, so they are safe to script against. Exit codes follow the [table in Usage details](#exit-codes) (`ok: false` ⇒ exit `1`).

`preferred_path_failed_alternate_reachable` is a separate, measured cause on `internet_tcp`: the reference TCP connections failed through the preferred default path while this run connected to the target through a distinct, lower-preference default path in the same address family. It requires known main-table routing decisions and a target route matching the successful socket source. Linux supplies this evidence for IPv4 and IPv6; platforms without the required table knowledge retain the legacy causes. The target may use a more-specific route through the alternate gateway. This proves that the alternate reaches that target, not that changing defaults will restore all connectivity or that the upstream failure has been localized. No additional network probes are sent.

Route evidence reflects platform measurement capabilities. Linux's IPv6 default-route classifier has no neighbor-failure callback. Darwin checks gateway resolution only with exactly one unscoped default. An unavailable observation leaves localization uncertain; it is not evidence that the gateway works. Linux ARP and Windows incomplete-neighbor state are interpreted after reference connection failure and can still reflect resolution in progress. Cross-builds check compilation, not native macOS or Windows runtime behavior.


A machine-readable [JSON Schema](https://github.com/heymaikol/network-doctor/blob/main/schema/report-v1.schema.json) for this document is published at `schema/report-v1.schema.json`, in Draft 2020-12. It describes the contract above as it already is rather than proposing a replacement for it: the report `--json` prints, the same object `--via` prints for a run performed on another machine, and each `--watch` line, which is that document plus `ts`. It deliberately permits properties it does not list, because the report is additive within v1 and a consumer pinned to the file has to keep validating against a newer netdoc. It freezes an enumeration only where this page publishes a closed vocabulary (`status`, `verdict`, `confidence`, the causal-evidence `kind`, `observation` and `reason`, the address-family states, and a route's `family`, `tunnel` and `reason`); the identities documented as growing over time, meaning finding and remediation ids, check ids, failure causes and tunnel kinds, stay open strings. Network Doctor validates its own reports against the file and checks every serialized field and every frozen vocabulary against it, so the published schema cannot fall behind the producer. The other machine-readable outputs are separate contracts the file does not describe: the [profile aggregate](#aggregate-result) (which carries reports of this shape inside it), a [comparison](#machine-readable-comparison), a [two-sided reading](#machine-readable-two-sided-reading), [peer output](#peer-json-output), and the [`.ndoc` snapshot](#snapshot-format-and-versioning).
...
| `recheck_connectivity_endpoint` | Read what the connectivity endpoint answered |
...
`options` records only the settings that decide what the probes did: the per-check timeout as an exact whole-millisecond count, which is the only precision `--timeout` accepts, so `probe_timeout_ms` is the run's own value and not a rounding of it (`0` means an artifact that does not record one, which is what a file written before the field existed decodes to), the second-opinion resolver (an IP address, empty when `--public-dns ""` switched that row off), the `--check`/`--skip` selection in the order it was given, and the `--iface` binding netdoc resolved. `public_dns_auto` is present and `true` when the run named no resolver, which is what let that row fall back to the other address family; it is an additive v1 field, absent when the resolver was named, so `public_dns` keeps the address-or-empty meaning every v1 reader already has. `check`, `skip`, and `source` are absent when the run used none of them. Flags that only affect presentation are not recorded, because a comparison never reads them.
...
The rest of a row's identity and the run's outcome are enforced the same way, and by the reader as well as the writer, so a hand-edited file is refused rather than read as something no run produced. Every check carries a non-empty `id`, no `id` appears twice, and `status` is one of the six values above and nothing else. `ok` is not an opinion the file holds beside its rows: it is `true` exactly when no row is `FAIL` and none is `INCOMPLETE`, so a run cannot call itself clean beside a failure or call itself broken with nothing in it that broke. Duplicate check ids matter most of the set, because different readers resolve them differently, and a file that can be read as two different runs is not evidence of either.

`deps` is the dependency graph the run executed, recorded per row so a reader can rebuild it without a live probe list, and it is enforced as a graph. Every entry names a check in the same file, no row lists the same dependency twice, and no row waits on itself, directly or through the rows it waits on: a probe runs only once everything it waits on has a result, so none of those could have been scheduled. The order of `checks` is the order the graph was built, and it is deliberately not required to be topological, because a row may name a dependency that appears further down the list and still be a row that ran. A dependency naming a check this build has never heard of is additive vocabulary, not a broken file, the same as a finding id.

`ran` answers a narrower question, and only that one: did the probe body execute. A skipped row and an incomplete row both report `ran: false`, so `ran` is not how a reader tells them apart; `status` is.

Both readers hold `ran` to the status beside it, because the two are not independent facts about a run. A `PASS`, `WARN`, or `FAIL` row carries an outcome a probe body measured, so it ran. A `SKIP` row was recorded when a prerequisite failed and an `INCOMPLETE` row was never reached at all, so neither did. `N/A` is deliberately outside the rule: a probe reports it from inside its own body after looking, and a netdoc that ruled a row inapplicable before calling it would still be writing a valid v1 file. A row that did not run also reports `duration_ms: 0`, because `ran` is that duration read as a yes or no. The reverse is not required, since a probe body faster than the unit can round to zero.

`observed.resolver_targets` holds the same attempted DNS service addresses as the JSON report. It is additive and optional, so an older snapshot with no field makes no resolver-provenance claim. `observed.routes` holds the same per-destination route decisions the JSON report documents above, in the same shape and with the same rule that an absent field is unknown rather than zero. It is an additive optional field: a snapshot written before it existed simply has none, and every reader treats that as "this run recorded no route decisions", never as "there was no route". `observed.connect_cleartext` mirrors the JSON report's `connect_cleartext`: `true` on a working `proxy_connect` row whose environment proxy used a plaintext HTTP CONNECT, and absent otherwise. It is additive and optional in the same way, it survives `--support` sanitization because a boolean names nobody, and an absent field never means a TLS hop to the proxy was confirmed or that no cleartext hostname was sent. Nothing stores a routing table; the entries are the answers to the bounded set of lookups the run already had to make.

Within a check, `observed` is what the probe measured and `derived` is what the cross-probe reasoning pass concluded about that row afterwards. Two fields live under `derived`. `status_downgraded` marks an observed egress failure that was relaxed to a Warn because another path carried traffic off this network on the same run: the endpoint under test answered a direct connection at a public address, or the environment proxy tunnelled. Its presence says the row's status is a conclusion rather than a reading. A destination reached without leaving the network, such as a device on the LAN or a peer in `100.64.0.0/10`, never earns one, and neither does the resolver answering. `answer_comparison` is on the `dns_public` row and records what that pass concluded when it compared the two resolvers' answers: `agree` or `disagree`, and absent when there was nothing to compare. It is stored because the comparison is made from the addresses the resolvers returned, and a `--support` artifact holds only pseudonyms of those, so a reader that recomputed it would be asking a different question than the run did. Absent never means the answers agreed, and an artifact written before the field existed has none. A row can therefore carry `derived` without its status having been rewritten. `cause`, `verdict`, and the finding IDs use exactly the vocabularies documented for [JSON output](#json-output) and [diagnosis findings](#diagnosis-findings).

The diagnosis has to resolve against those rows. Every finding carries a non-empty `id`, no `id` appears twice, and a non-empty `blamed`, `failed_stage`, finding `focus`, or `evidence` entry names a check that is actually in the file. `failed_stage` is derived the way `ok` is: it is the first `FAIL` row in run order, and it is absent exactly when no row failed. Where a finding carries both `evidence` and `causal_evidence`, the first has to be the compatibility projection of the second, meaning the unique observed check ids in reasoning order with the `not_evaluated` entries left out, so an old consumer and a new one cannot be told different things about what a conclusion rests on. A snapshot written before typed evidence existed carries only `evidence` and is not asked to invent the rest. A finding `id` this build has never heard of is additive vocabulary, not a broken file, and is not refused. A `support`, `contradiction`, or `ruled_out` entry is a claim about what a row observed, so it has to name a row that ran: a status and an observation can still line up on a row whose probe body never executed, and a claim built from that would rest on nothing measured. The `not_evaluated` entries are the exception and keep their own meaning, because saying a row was never evaluated is the one claim that does name an unmeasured row.
...
Identity is read semantically, not spelled. Two runs name one endpoint when their port and protocol agree and their hosts are the same name: a hostname is matched the way DNS reads it, ignoring case and a terminal root dot, so `example.com`, `EXAMPLE.COM` and `example.com.` are one target, and an IP literal is matched as an address, so the compressed and expanded spellings of one IPv6 address are one target too. An address family is not a spelling, so an IPv4 address and its IPv4-mapped IPv6 form stay two targets. The spellings themselves are still reported as changes, since what each run recorded is a real difference between the two files; it is `same_target` that reads through them.
...
| `--timeout` | Accepted. The same positive whole-millisecond per-probe timeout is sent to both runs. |
...
| `--iface` | Rejected. One local interface name or source IP cannot honestly identify corresponding interfaces on two different machines. Use two separately saved snapshots when each side needs its own binding; the offline reading reports a differing binding as a [caveat](#caveats). |
...
- **Source bindings that did not constrain the two runs alike.** One run bound its probes to a chosen local source with `--iface` while the other used the machine's own routing choice, so a row that fails on one side may follow from that binding rather than from the machine. Two runs that were both bound are also reported when their bindings did not cover the same address families, because a source with no address for a family means that family was never attempted rather than unreachable. The binding is read as policy and never as identity: probes bind by the resolved addresses, and two machines have different interface names and different local addresses by construction, the same way they have different hostnames, so differing values are not a caveat and neither the names nor the addresses appear in the caveat text. This reading is where independently bound snapshots meet, since live `--two-sided --via` rejects `--iface`.
...
The request carries the run settings as the user spelled them: target, `--iface`, `--public-dns`, `--timeout`, `--check`, and `--skip`. `timeout_ms` has held whole milliseconds since protocol 1, which is why restricting `--timeout` to that precision rather than adding a field keeps all three version pairings exact: a current netdoc sends only values an older worker already reconstructs unchanged, an older netdoc's whole-millisecond value is honored here as it always was, and neither side approximates the other's budget. A `timeout_ms` that no `time.Duration` can hold is refused before it is converted, so an out-of-range number cannot wrap into a small positive timeout. Nothing is pre-resolved locally, because the machine that will run the probes is the one that has to resolve it. Probe restrictions also travel inside `skip`: `--no-reference-egress` is resolved here into the probe IDs it removes, and an unknown-protocol target adds `path_mtu` unless explicitly requested with `--check path_mtu`. This keeps older workers from transmitting the PMTU payload by default. That is deliberate: skip has been honored since protocol 1, whereas a new request field would be ignored by any remote netdoc old enough not to know it, and a safety guarantee that a mismatched pair silently drops is worse than no flag at all. `--iface` is the clearest case for an ordinary remote run: it names an interface on the SSH host, so the local machine has no business resolving it against its own NICs. Live two-sided mode reuses this protocol unchanged for its remote half, but rejects `--iface` because the same spelling cannot also select a corresponding local interface honestly.
...
What the local side does check is that the two artifacts describe the same run, before either of them reaches anything else. They are spent separately: the report decides what is printed and what the process exits with, and the snapshot is what `--save` writes to a file for a later comparison, so a pair that disagreed would leave a machine exiting `0` over a stored artifact recording a failure, with no way for a later reader to tell which half was the run. The snapshot is held to the `.ndoc` rules, which nothing has applied to it yet because it arrived inside the envelope rather than as a file, and then the overlap is compared: the build that answered, the target, `ok`, the checks in order with their statuses and causes, the verdict, the first failed check, and each finding's identity, focus, confidence, evidence, typed causal evidence and counterfactual. A disagreement is reported as a protocol failure naming the field and both readings of it.

Only the overlap, and only its machine-readable part. The two formats are different contracts on purpose: the report carries remediation the snapshot does not, the snapshot carries observations and run options the report does not, and the derived sentences in both are documented as never parsed back. Nor does this check rely on refusing unknown fields. Both artifacts are decoded into the fields this build knows, so a newer remote's additions are already gone before the comparison runs, and protocol 1 stays forward compatible with them.
